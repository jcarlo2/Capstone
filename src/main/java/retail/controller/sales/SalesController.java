...
public class SalesController {
    ...
    public SalesController(@NotNull RightCenterPanel rightCenterPanel, @NotNull LeftBorderPanel leftBorderPanel) {
        table = rightCenterPanel.getSalesReport().getAddSalesReport().getCenterTable();
    ...
    private void saveReportInDatabase() {
        addPanel.getSave().addActionListener(e -> {
            if(e.getSource() == addPanel.getSave()) {
                ...
                if(salesController.ifReportExist(addPanel.getReportId().getText())) {
                    ConstantDialog.GENERATE_NEW_REPORT_ID();
                    return;
                }
                salesController.addReport(createSalesReport(),getAllItemReportAtSalesTable());
    ...
    private void addItemToSalesTable() {
        addPanel.getAdd().addActionListener(e -> {
            if(isAllAddFieldValid()) {
                ...
                table.addRow(new SalesReportItemObject(data[0],new BigDecimal(data[1]),
    ...
    private String generateReportId() {
        ...
        while(flag) {
            ...
            if(!salesController.ifReportExist(formatId)) flag = false;
        }
        return formatId;
    }
